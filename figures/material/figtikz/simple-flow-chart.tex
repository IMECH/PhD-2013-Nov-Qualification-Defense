...
\begin{tikzpicture}
    ...
    \node [block](C) at (3,3) {方法};
    \node [block](D) at (3,-3) {并行};

    \node [block](E) at (6,1) {高分子};
    \node [block](F) at (6,-1) {红细胞};
    \node [block](G) at (6,3) {\textcolor{gray}{MD, DPD, SPH藕合}};
    \node [block](H) at (6,-3) {CPU};

    \node [block](I) at (9,1) {\textcolor{gray}{多物理场}};
    \node [block](J) at (9,-1) {\textcolor{gray}{乳腺癌细胞}};
    \node [block](K) at (9,-3) {\textcolor{gray}{GPU}};

    \node [block](L) at (12,0) {\textcolor{gray}{高性能大规模计算}};

    \node [cloud](M) at (3,1.5) {\textcolor{gray}{改进势函数}};
